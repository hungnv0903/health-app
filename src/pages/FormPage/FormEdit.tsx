...
import { fromSchema } from "./validate";
...
const FormEdit = () => {
  ...
  useEffect(() => {
    ...
    if(dataFormLocalstorage!==null){
      ...
      const user = arrDataFormLocalstorage.find((item:any)=>item.id===userId) ; 
    ...
    const fetchDataProvince = async () => {
      await axios.get(URL_API, {
        ...
        .then((response) => setProvinceData(response.data.results))
  ...
  useEffect(()=>{
        setSelectProvinceId(userEdit?.province)
        setSelectProvinceName(userEdit?.provinceName)
  ...
  useEffect(() => {
    ...
    if(provinceData.length>0){ 
      const selectprovince = provinceData.find((item:any)=>item.province_id===selectProvinceId) ; 
  ...
  const handleFormData = (values:any)=>{
    const newValue = {
      ...values , province:selectProvinceId ,provinceName:selectProvinceName ,
    } 

    const newDataform = dataForm.filter((item:any)=>item.id !== userId) ; 
    const DataformEdit = [...newDataform , newValue] ; 
    localStorage.setItem('Formdata',JSON.stringify(DataformEdit)) ; 
  }
              ...
              <div className="d-flex">
                ...
                <div className="me-4">
                  <button type="button" className="btn btn-danger fs-4" onClick={()=>{
                    const popup = confirm("Do you want to cancel?") ; 
                    if(popup){
                      navigate('/table')
                    }
                    }}>Cancel</button>
                ...
                <div className="me-4">
                  <button type="button" className="btn btn-secondary fs-4" onClick={()=>{
                    const popup = confirm("Do you want to reset?") ;; 
                    if(popup){
